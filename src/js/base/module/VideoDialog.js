  ...
  initialize() {
    ...
    const body = [
      '<div class="form-group note-form-group row-fluid">',
        ...
        `<label for="note-dialog-video-file" class="note-form-label"> ${this.lang.image.selectFromFiles} </label>`,
        `<input id="note-dialog-video-file" class="note-video-input form-control-file note-form-control note-input" `,
        `type="file" name="files" accept="video/*" />`,
        `<div class="during-upload" style="display:none;">
          <p><span id="percent-transferred"></span>% done (<span id="bytes-transferred"></span>/<span id="total-bytes"></span> bytes)</p>
          <progress id="upload-progress" max="1" value="0"></progress>
        </div>`,
        `<br><input type="button" class="btn btn-primary note-btn note-btn-primary " value="Upload Youtube" id="btn-upload-youtube"> <div class="processing-bar"> </div> `,
      '</div>',
    ].join('');
